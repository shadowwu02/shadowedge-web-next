...
export function ModelSelector({
  ...
  return (
    <section className="rounded-[22px] border border-white/10 bg-white/[.055] p-3">
      ...
      <select
        className="w-full rounded-2xl border border-white/10 bg-[#10141f] px-3 py-2.5 text-sm font-bold text-white outline-none transition focus:border-[#ffb44d]/70"
        onChange={(event) => {
      ...
      {selected ? <p className="mt-2 line-clamp-1 text-xs leading-5 text-white/48">{selected.desc}</p> : null}
    </section>
  );
}
