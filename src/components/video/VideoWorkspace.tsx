...
import { remakeFeatures } from "@/config/remakeFeatures";
import {
...
function buildFullEpisodeStoryboardFromResult(input: {
  ...
  if (!result && !canonicalResult) return null;

  const rawShotList = canonicalResult?.shots?.length ? canonicalResult.shots : Array.isArray(result?.shotList) ? result.shotList : [];
  const rawChunks = canonicalResult?.chunks?.length ? canonicalResult.chunks : Array.isArray(result?.chunks) ? result.chunks : [];
  ...
  const shots: RemakeShot[] = rawItems.map((item, index) => {
    ...
    const shotNumber = Math.max(1, Math.round(getRecordNumber(record, ["globalShotIndex", "shotIndex", "shot", "beatIndex"], index + 1)));
...
function buildFullEpisodeResultFromCanonical(canonicalResult?: VideoAnalysisCanonicalResult | null): RemakeEpisodeResult | undefined {
  if (!canonicalResult) return undefined;

  return {
    analysisSource: "fallback",
    chunks: canonicalResult.chunks.map((chunk) => ({
      chunkIndex: chunk.chunkIndex,
      coverage: chunk.coverage
        ? {
            actualShotCount: chunk.coverage.actualShotCount,
            coverageRatio: chunk.coverage.coverageRatio,
            durationSeconds: chunk.coverage.durationSeconds,
            firstTimestamp: chunk.coverage.firstTimestamp,
            gapCount: chunk.coverage.gapCount,
            lastTimestamp: chunk.coverage.lastTimestamp,
            ok: chunk.coverage.ok,
            reason: chunk.coverage.reason,
            recommendedMinShotCount: chunk.coverage.recommendedShotCount,
          }
        : undefined,
      duration: chunk.duration,
      end: chunk.end,
      id: chunk.id || `chunk_${chunk.chunkIndex}`,
      shotBeatCount: chunk.shotCount,
      start: chunk.start,
      status: chunk.status,
    })),
    coverage: {
      actualShotCount: canonicalResult.coverage.actualShotCount,
      coverageRatio: canonicalResult.coverage.coverageRatio,
      durationSeconds: canonicalResult.coverage.durationSeconds,
      firstTimestamp: canonicalResult.coverage.firstTimestamp,
      gapCount: canonicalResult.coverage.gapCount,
      lastTimestamp: canonicalResult.coverage.lastTimestamp,
      ok: canonicalResult.coverage.ok,
      reason: canonicalResult.coverage.reason,
      recommendedMinShotCount: canonicalResult.coverage.recommendedShotCount,
    },
    episode: true,
    mode: "full_episode",
    mock: canonicalResult.visualUnderstanding.mock,
    note: canonicalResult.storyboard.summary,
    providerCallMade: canonicalResult.visualUnderstanding.providerCallMade,
    realVisualUnderstanding: canonicalResult.visualUnderstanding.realVisualUnderstanding,
    remakePlan: canonicalResult.remakePlan.map((item) => item.prompt || item.title),
    shotList: canonicalResult.shots,
    summary: canonicalResult.storyboard.summary,
    timeline: canonicalResult.timeline,
    vlmCalled: canonicalResult.visualUnderstanding.vlmCalled,
  };
}

function buildLongVideoStoryboardFromCanonical(input: {
  canonicalResult?: VideoAnalysisCanonicalResult | null;
  fallbackReason: string;
  sourceTitle: string;
  targetRatio?: string;
...
function buildParamsForModelAndReferences(
  model: VideoModel,
...
export function VideoWorkspace() {
  ...
  const remakeJobRecoveryRef = useRef("");
  ...
  const applyCompletedFullEpisodeAnalysisJob = useCallback(
    (
      fullEpisodeJob: VideoRemakeFullEpisodeAnalysisJob,
      options: {
        saveDraft?: boolean;
      ...
      const fullEpisodeResult = fullEpisodeJob.result as RemakeEpisodeResult | null;
      const canonicalEpisodeResult = buildFullEpisodeResultFromCanonical(fullEpisodeCanonical);
      ...
      const fullEpisodeFallbackReason =
        ...
        fullEpisodeCanonical?.storyboard.summary ||
        t("video.remake.fullEpisode.betaWarning");
      ...
      if (options.saveDraft && options.settings) {
  ...
  const applyCompletedLongVideoAnalysisJob = useCallback(
    (longVideoJob: VideoRemakeLongAnalysisJob) => {
      const canonicalResult = longVideoJob.canonicalResult;
      const jobMetadata = getPlainRecord(longVideoJob.metadata);
      const resultMetadata = getPlainRecord(longVideoJob.result?.metadata);
      const targetRatio = normalizeRemakeTargetRatio(
        longVideoJob.result?.storyboard?.targetRatio ||
          longVideoJob.result?.storyboard?.aspectRatio ||
          resultMetadata.targetRatio ||
          resultMetadata.aspectRatio ||
          jobMetadata.targetRatio ||
          jobMetadata.aspectRatio ||
          params.ratio,
      );
      const sourceTitle = remakeSourceVideo?.name || "Long source video";
      const storyboard =
      ...
      if (!storyboard?.shots?.length) throw new Error("LONG_VIDEO_STORYBOARD_UNAVAILABLE");

      const canonicalVisual = canonicalResult?.visualUnderstanding;
      const providerCallMade = hasTrueFlag(
        canonicalVisual?.providerCallMade,
        jobMetadata.providerCallMade,
        resultMetadata.providerCallMade,
        storyboard.providerCallMade,
      );
      const vlmCalled = hasTrueFlag(
        canonicalVisual?.vlmCalled,
        jobMetadata.vlmCalled,
        resultMetadata.vlmCalled,
        storyboard.vlmCalled,
      );
      const analyzedStoryboard: RemakeStoryboard = {
        ...storyboard,
        analysisSource: "vlm",
        aspectRatio: storyboard.aspectRatio || targetRatio,
        ...
        shots: storyboard.shots.map((shot) => {
          const imageHints = shot.referenceHints.images.filter(
            (hint) => hint !== "no real visual model was called",
          );
          return {
            ...shot,
            referenceHints: {
              ...shot.referenceHints,
              images: imageHints.length ? imageHints : ["source keyframe analysis"],
            },
          };
        }),
      ...
      if (!draftResult.ok) setWorkspaceNotice(t("video.remake.draftSaveFailed"));
    },
    [
      params.ratio,
  ...
  const handleLongVideoJobRestored = useCallback(() => {
    setWorkspaceMode("remake");
    setRemakeMode("long_video");
    setRemakeAnalysisError("");
    setRemakeAnalysisNotice("");
  ...
  const {
    activeJob: activeLongVideoAnalysisJob,
    cancelConfirmation: cancelLongVideoConfirmation,
    confirmCreate: confirmLongVideoAnalysis,
    errorCategory: longVideoAnalysisErrorCategory,
    isActive: isLongVideoAnalysisActive,
    isCreating: isLongVideoAnalysisCreating,
    isEstimating: isLongVideoAnalysisEstimating,
    pendingEstimate: pendingLongVideoEstimate,
    progress: longVideoAnalysisProgress,
    requestEstimate: requestLongVideoAnalysisEstimate,
  ...
  const handleAnalyzeRemakeStoryboard = useCallback(async () => {
    ...
    try {
      ...
      if (remakeMode === "long_video") {
        ...
        const longVideoAnalysisSource =
          longVideoCanonical?.analysisSource === "vlm"
            ? "vlm"
            : longVideoCanonical?.analysisSource === "sandbox"
              ? "sandbox_vlm"
              : getRemakeAnalysisSource(longVideoStoryboard.analysisSource) || "fallback";
        const longVideoVlmCalled = hasTrueFlag(
          longVideoCanonicalVisual?.vlmCalled,
        ...
        const longVideoProviderCallMade = hasTrueFlag(
          longVideoCanonicalVisual?.providerCallMade,
        ...
        const longVideoSandboxVlm = hasTrueFlag(
          longVideoCanonicalVisual?.sandbox,
      ...
      if (remakeMode === "full_film") {
        ...
        applyCompletedFullEpisodeAnalysisJob(fullEpisodeJob, {
          saveDraft: true,
          settings,
      ...
      if (remakeMode === "full_film") {
        if (fullEpisodeJobIdForRecovery) {
          try {
            const recoveredJob = await getFullEpisodeRemakeAnalysisStatus(fullEpisodeJobIdForRecovery);
            if (remakeSourceRevisionRef.current !== analysisRevision) return;
            if (recoveredJob.status === "completed") {
              applyCompletedFullEpisodeAnalysisJob(recoveredJob, {
                saveDraft: true,
            ...
            console.warn("[ShadowEdge Next] full episode completed recovery failed:", recoveryError);
          }
        }
        setRemakeStoryboard(null);
        setRemakeAnalysisMeta(null);
    ...
    remakeTargetRegion,
    remakeTranslateDialogue,
    requestLongVideoAnalysisEstimate,
    t,
    tf,
  ]);

  const handleConfirmLongVideoAnalysis = useCallback(async () => {
    setRemakeAnalysisError("");
    setRemakeAnalysisNotice("");
    await confirmLongVideoAnalysis();
  }, [confirmLongVideoAnalysis]);

  const handleCancelLongVideoConfirmation = useCallback(() => {
    cancelLongVideoConfirmation();
    setRemakeAnalysisNotice(t("video.remake.longVideo.cost.confirmationCancelled"));
  }, [cancelLongVideoConfirmation, t]);
  ...
  const handleRemakeSourceVideoChange = useCallback(
    (source: RemakeSourceVideo | null) => {
      if (isLongVideoAnalysisActive || isLongVideoAnalysisCreating) {
        setWorkspaceNotice(t("video.remake.longVideo.analyzing"));
        return;
      }
      if (!source) {
    ...
    [isLongVideoAnalysisActive, isLongVideoAnalysisCreating, remakeSourceVideo, resetRemakeDerivedSourceState, t],
  );
  ...
  }, [isLongVideoAnalysisActive, isLongVideoAnalysisCreating, resetRemakeDerivedSourceState, t]);
  ...
  const handleRemakeModeChange = useCallback(
    ...
    [isLongVideoAnalysisActive, isLongVideoAnalysisCreating, remakeMode, resetRemakeDerivedSourceState, t],
  );
  ...
  const generateButtonLabel = useMemo(() => {
    ...
    if (generatedAudioReferenceIssue) return t("video.actions.turnOffAudio");
  ...
  }, [audioTupleInvalid, catalogStatus, estimatedCredits, generatedAudioReferenceIssue, hasEnoughCredits, internationalExecutionUnavailable, isProcessing, isSignedIn, isUploadingMedia, t, tf, token, tuplePricingReady]);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      if (!hasExistingDraft) {
        ...
        setParams(
          buildParamsForModelAndReferences(draftModel, {
  ...
  const handleFillFromHistory = useCallback(
    (record: (typeof history)[number]) => {
      ...
      const nextParams = buildParamsForModelAndReferences(fillModel, {
  ...
  }, [guardedLongVideoUxVisible, remakeLongVideoCostEstimate, remakeMode, remakeSourceVideo, t, tf]);
        ...
        <div className="se-subtle-scrollbar grid min-h-0 min-w-0 flex-1 content-start gap-3.5 overflow-x-hidden overflow-y-auto p-3.5">
          {comingSoonCapability ? (
            ...
            <VideoRemakeWorkspace
              ...
              onCancelLongVideoConfirmation={handleCancelLongVideoConfirmation}
      ...
      <main className="flex min-h-[560px] min-w-0 flex-col overflow-hidden xl:min-h-0">
        {workspaceMode === "remake" ? (
          <RemakeStoryboardPanel
            ...
            longVideoSourceName={remakeSourceVideo?.name || ""}
