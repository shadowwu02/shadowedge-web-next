  ...
  return (
    <div className="h-full overflow-y-auto rounded-[28px] border border-[#d9b56d]/15 bg-[#090909]">
      <div className="mx-auto max-w-6xl space-y-6 p-5 md:p-8">
        <header className="flex flex-col gap-5 rounded-[26px] border border-[#d9b56d]/20 bg-[radial-gradient(circle_at_top_right,rgba(217,181,109,.14),transparent_42%),#11100e] p-6 md:flex-row md:items-end md:justify-between">
          <div>
            <div className="flex flex-wrap items-center gap-2 text-[11px] font-black uppercase tracking-[.18em] text-[#f2d899]">
              ...
              <span className="rounded-full border border-emerald-400/25 bg-emerald-400/10 px-2.5 py-1 text-emerald-200">Read only</span>
              <span className="rounded-full border border-white/10 px-2.5 py-1 text-white/50">Excluded from analytics</span>
            </div>
            <h1 className="mt-4 text-3xl font-black tracking-tight text-white md:text-4xl">{demo.name}</h1>
            <p className="mt-3 max-w-2xl text-sm leading-6 text-white/55">
              Explore a complete example from creative planning through delivery. This fixture never writes to your projects or starts generation.
            </p>
          </div>
        ...
        <section className="grid gap-4 md:grid-cols-4" aria-label="Demo creative canvas">
        ...
        <div className="grid gap-5 lg:grid-cols-[1.35fr_.65fr]">
          <section className="rounded-[24px] border border-white/10 bg-white/[.025] p-5">
            <div className="flex items-center justify-between">
              <div>
                <span className="text-[10px] font-black uppercase tracking-[.18em] text-[#d9b56d]">Storyboard + Timeline</span>
                <h2 className="mt-2 text-xl font-black text-white">Three-shot campaign sequence</h2>
              </div>
              <span className="text-xs text-white/42">12 seconds</span>
            </div>
            <div className="mt-5 grid gap-3 md:grid-cols-3">
              {demo.storyboard.map((shot, index) => (
                <article className="rounded-2xl border border-white/8 bg-black/30 p-4" key={shot.id}>
                  <div className="aspect-video rounded-xl border border-[#d9b56d]/10 bg-[linear-gradient(145deg,#18140e,#080808)] p-3 text-[10px] font-black text-[#d9b56d]/80">SHOT 0{index + 1}</div>
          ...
          <div className="space-y-5">
            <section className="rounded-[24px] border border-emerald-400/15 bg-emerald-400/[.045] p-5">
              <span className="text-[10px] font-black uppercase tracking-[.18em] text-emerald-300">Review example</span>
            ...
            <section className="rounded-[24px] border border-[#d9b56d]/20 bg-[#d9b56d]/[.045] p-5">
              <span className="text-[10px] font-black uppercase tracking-[.18em] text-[#d9b56d]">Delivery example</span>
              <h2 className="mt-3 text-xl font-black text-white">Package {demo.delivery.version}</h2>
              <p className="mt-2 text-sm text-white/50">Status: {demo.delivery.status}</p>
              <p className="mt-4 rounded-xl border border-white/8 bg-black/25 p-3 text-xs leading-5 text-white/42">
                Preview only. No external sharing, publishing, generation, runtime, credits, or billing actions are available here.
              </p>
            </section>
          </div>
        </div>
