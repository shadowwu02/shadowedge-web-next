...
export function normalizeImageModel(rawModel: unknown): ImageModel {
  ...
  return {
    ...
    availability: String(raw.availability || (raw.available === false ? "maintenance" : "available")),
    maintenanceMessage: String(raw.maintenanceMessage || raw.maintenance_message || ""),
    ...
    capabilities: {
      ...
      maxPromptLength: Math.max(1, pickNumber(capabilities.maxPromptLength ?? raw.maxPromptLength, 4000)),
      maxBatchCount,
