...
describe("FluxProxy International frontend contract", () => {
  ...
  it("requires ACTIVE review for the exact public binding profile", () => {
    const active = media("image", "i");
    expect(getFluxProxyReviewSummary([active], model.referenceBindingProfileId).ready).toBe(true);
    expect(getFluxProxyReviewSummary([{ ...active, providerAssetReview: { ...active.providerAssetReview!, referenceBindingProfileId: "rbp_6f4c9a2e13d847b0" } }], model.referenceBindingProfileId)).toMatchObject({ ready: false, modelMismatch: 1 });
    expect(getFluxProxyReviewSummary([{ ...active, providerAssetReview: { ...active.providerAssetReview!, isCurrent: false, status: "FAILED" } }], model.referenceBindingProfileId)).toMatchObject({ ready: false, failed: 0, staleAuthority: 1 });
  });
