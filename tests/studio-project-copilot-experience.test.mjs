...
test("Project Copilot schema exposes the bounded suggestion types", () => {
  ...
  assert.equal(studioCopilotDraftLabel("QUALITY_IMPROVEMENT_DRAFT"), "Quality Improvement Draft");
  assert.equal(studioCopilotDraftLabel("OPTIMIZATION_DRAFT"), "Optimization Draft");
  ...
  assert.equal(studioCopilotDraftLabel("SCENARIO_DECISION_DRAFT"), "Scenario Decision Draft");
  assert.equal(studioCopilotDraftLabel("DECISION_PATTERN_REVIEW_DRAFT"), "Decision Pattern Review Draft");
  assert.equal(studioCopilotActionLabel("PROJECT_ACTION_DRAFT"), "Project action draft");
  assert.equal(studioCopilotDraftLabel("PROJECT_ACTION_DRAFT"), "Project Action Draft");
  assert.equal(studioCopilotActionLabel("CANVAS_AUTO_PLAN_DRAFT"), "Canvas auto plan draft");
  assert.equal(studioCopilotDraftLabel("CANVAS_AUTO_PLAN_DRAFT"), "Canvas Auto Plan Draft");
  assert.equal(studioCopilotActionLabel("CANVAS_WORKFLOW_OPTIMIZATION_DRAFT"), "Canvas workflow optimization draft");
  assert.equal(studioCopilotDraftLabel("CANVAS_WORKFLOW_OPTIMIZATION_DRAFT"), "Canvas Workflow Optimization Draft");
});
...
test("Creative Copilot Panel displays Context, Workflow, suggestions, and Task status", () => {
  ...
  assert.match(component, /creativePatterns\.total/);
  assert.match(component, /experienceContext\.experienceIds/);
  assert.match(component, /creativePreferences\.total/);
  ...
  assert.match(component, /scenarios\.total/);
  assert.match(component, /scenarioContext\.scenarioCount/);
  assert.match(component, /decisionPatterns\.total/);
  assert.match(component, /decisionPatternContext\.patternCount/);
